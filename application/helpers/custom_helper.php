...
if (!function_exists('getDefaultLanguage')) {
    ...
    function getDefaultLanguage() {
        ...
        $default_language = $CI->Common_model->customGet($option);
    ...
    function send_push_notifications($msg, $user_id, $params) {
        ...
        if (!empty($device_history)) {
            foreach ($device_history as $dh) {
                ...
                $device_badges = $dh->device_badges;
...
if (!function_exists('CommonGet')) {
    ...
    if (!function_exists('allGroups')) {

        function allGroups($data = NULL) {
            $groups = array('birthday' => "Birth Day",
                'above_18' => "Above 18 years",
                'less_18' => "Less 18 years"
            );
            if ($data != NULL)
                return $groups[$data];
            else
     ...
     * @param  [int] $page_no
     */
    if (!function_exists('get_offsets_new')) {

        function get_offsets_new($page_no = 0) {
            $offset = ($page_no == 0) ? 0 : (int) $page_no * 30 - 30;
            return $offset;
        }
    ...
    if (!function_exists('android_push_notification')) {
        ...
        function android_push_notification($data_array = array(), $target_token) {
            $fields = array(
                'to' => $target_token,
            ...
            if ($resp->success == 1) {
                log_message('ERROR', "FCM - Message send successfully, message - " . $data_array['body']);
            } else {
                log_message('ERROR', "FCM - Message failed, message - " . $data_array['body']);
            }
            // Close connection
            curl_close($ch);
    ...
    if (!function_exists('ios_push_notification')) {
        ...
        function ios_push_notification($body = array(), $target_token) {
     ...
     * @param  [int] $page_no
     */
    if (!function_exists('getFloorDetail')) {

        function getFloorDetail($id) {
            $CI = & get_instance();
            $option = array('table' => 'mw_rooms', 'where' => array('id' => $id), 'single' => true);
            $floors = $CI->Common_model->customGet($option);
            return (!empty($floors)) ? $floors->name : "";
        }

    }
    if (!function_exists('getStringFormate')) {

        function getStringFormate($str) {
            if (strlen($str) > 120) {
                $content = $str;
                $htm = mb_substr($str, 0, 120, 'UTF-8') . '...<br>';
                $enc = str_replace("=", "", base64_encode($content));
                $htm .= '<a style="cursor:pointer" onclick="show_message(' . $enc . ')">view</a>';
                return $htm;
    ...
    if (!function_exists('dateFormateManage')) {

        function dateFormateManage($date) {
            return date(DEFAULT_DATE, strtotime($date));
        }

    }

    if (!function_exists('timeFormateManage')) {

        function timeFormateManage($time) {
            return date('H:i', strtotime($time));
        }

    }
    if (!function_exists('getSpecialRequest')) {

        function getSpecialRequest($id) {
            $CI = & get_instance();
            $special_request_id = json_decode($id);
            $str = "";
            if (!empty($special_request_id)) {
                foreach ($special_request_id as $rowss) {
                    $options = array('table' => SPECIAL_REQUEST, 'select' => '*', 'where' => array('id' => $rowss), 'single' => true);
                    $special_request = $CI->Common_model->customGet($options);
                    if (!empty($special_request)) {
                        $str .= $special_request->name . "<br>";
                    }
                }
                return $str;
            } else {
    ...
    if (!function_exists('paymentStatus')) {
        ...
        function paymentStatus($name,$payment,$id) {
            $str = "";
            if ($payment > 0) {
                $str = $name."<br/>";
                $str .= "<a onclick='open_other_modal(\"tablebooking\",\"open_paymentview_model\",\"".encoding($id)."\")' class='green_paid'>paid</a>";
            } else{
                $str = $name."<br/>";
                $str .= "<a onclick='open_other_modal(\"tablebooking\",\"open_payment_model\",\"".encoding($id)."\")' class='red_unpaid'>unpaid</a>";
            } 
            return $str;
        }
    ...
    if (!function_exists('getStatusStrBooking')) {

        function getStatusStrBooking($status,$id) {
            $str1 = "";
            $str2 = "";
            $str3 = "";
            $str4 = "";
            ...
            $enc .= encoding($id);
            $enc .= "'";
            $htm = '<select id="status" name="status" size="1" class="form-control" onchange="bookingStatusAuth('.$enc.', this)">';
            $htm .= '<option value="1" '.$str1.'>Confirm</option>';
            $htm .= '<option value="2" '.$str2.'>Pending</option>';
            $htm .= '<option value="3" '.$str3.'>Cancel</option>';
            $htm .= '<option value="4" '.$str4.'>Arrived</option>';
            $htm .= '<option value="5" '.$str5.'>No Show</option>';
            $htm .= '</select>';
            return $htm;
        }
        ...
        if (!function_exists('getStatusStrBookingDashboard')) {

        function getStatusStrBookingDashboard($status,$id) {
            $str1 = "";
            $str2 = "";
            $str3 = "";
            $str4 = "";
            ...
            $enc .= encoding($id);
            $enc .= "'";
            $htm = '<select id="status" name="status" size="1" class="form-control" onchange="bookingStatusAuthDashboard('.$enc.', this)">';
            $htm .= '<option value="1" '.$str1.'>Confirm</option>';
            $htm .= '<option value="2" '.$str2.'>Pending</option>';
            $htm .= '<option value="3" '.$str3.'>Cancel</option>';
            $htm .= '<option value="4" '.$str4.'>Arrived</option>';
    ...
    if (!function_exists('getConfirmStr')) {

        function getConfirmStr($status,$id) {
            $str = "";
            if ($status == 1) {
               return $str = "Confirm";
            }
            if ($status == 2) {
                $url = base_url() . 'tablebooking/index/' . encoding($id);
                $str = "<a href='$url' class='btn btn-md btn-primary'><i class= 'fa fa-arrow-circle-right fa-lg'></i></a>";
            } else {
                $url = base_url() . 'tablebooking/index/' . encoding($id);
                $str = "<a href='$url' class='btn btn-md btn-primary'><i class= 'fa fa-arrow-circle-right fa-lg'></i></a>";
            }
    ...
    if (!function_exists('encodingStr')) {

        function encodingStr($id) {
            $ht = '"';
            $ht .= encoding($id);
            $ht .='"';
            return $ht;
     ...
     if (!function_exists('encodingStrAction')) {

        function encodingStrAction($id) {

            $id= encoding($id);
            $htm = "<a href='" . base_url() . "tablebooking/index/$id'  class='' type='button'><i class='fa fa-edit fa-lg'></i></a> "
                . "<a href='javascript:void(0)' onclick='viewFn(\"tablebooking\",\"open_model\",\"$id\")' class='text-success' type='button'> <i class='fa fa-eye fa-lg'></i></a>"
                . "<a href='javascript:void(0)' onclick='delBooking(\"$id\")' class='text-danger' type='button'> <i class='fa fa-trash-o fa-lg'></i></a>"
                . "<a href='" . base_url() . "transferBooking/index/$id' title='Move Now' class='text-info' type='button'> <i class='fa fa-arrows fa-lg'></i></a>";
            return $htm;
            }
