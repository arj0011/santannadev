...
class Feedback extends Common_Controller {
    public $data = array();
    public $file_data = "";
    public $_table = FEEDBACK_OF_STORE;
    ...
    public function index() {
                $this->data['url'] = base_url().'feedback';
        $this->data['pageTitle'] = lang('feedback');
        $this->data['parent'] = "feedback";
        $option = array('table' => $this->_table.' as feedback',
                        'select' => 'feedback.id,feedback.company_name,feedback.surname,feedback.store_name,'
                                   . 'feedback.name,feedback.email,feedback.contact,feedback.feedback,feedback.feedback_time,'
                                   . 'feedback.sorting_order',
                        //'join' => array(SERVICE_CATEGORY.' as catgory' => 'catgory.id=service.store_category_id')
            );
       
        if(getDefaultLanguage() == "el"){
            $option['select'] = 'feedback.id,feedback.company_name,feedback.surname,feedback.store_name,'
                                   . 'feedback.name,feedback.email,feedback.contact,feedback.feedback,feedback.feedback_time,'
                                   . 'feedback.sorting_order';
        }
        ...
        if ($this->form_validation->run() == true) {
                ...
                if ($this->filedata['status'] == 0) {
                   ...
                   $options_data = array(
                        ...
                        'store_lat'         => $this->input->post('latitude'),
                        'store_lang'        => $this->input->post('longitude'),
