...
class Users extends Common_Controller {
    ...
    public function index() {
        ...
        $this->data['parent'] = "User";
              $option = array(
            'table' => USERS. ' AS u', 
            'select' => 'u.name as user_name,u.user_image,u.id,u.email,u.date_of_birth,u.address,u.created_date,u.is_deactivated,u.qr_image,uw.total_point', 
            'order' => array('u.id' => 'DESC')
            );
        if (getDefaultLanguage() == "el") {
            $option['select'] = 'u.name as user_name,u.user_image,u.id,u.email,u.date_of_birth,u.address,u.created_date,u.is_deactivated,u.qr_image,uw.total_point';
        }
        $option['join'] = array(array(USERPOINTSWALLET.' AS uw','uw.user_id = u.id','left'));
