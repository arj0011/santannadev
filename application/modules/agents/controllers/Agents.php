...
class Agents extends Common_Controller {
     ...
     */
     public function index() {
        ...
        $this->data['url'] = base_url().'agents';
        $this->data['pageTitle'] = "Agent";
        $option = array('table' => AGENTS.' as agent', 
            'select' => 'agent.full_name,agent.image,agent.id,agent.email,agent.date_of_birth,agent.phone_number,agent.create_date,agent.is_active,role.name',
            'join' => array(ROLE.' as role' =>'role.id=agent.role_id' ),
            'order' => array('agent.id'=>'DESC')
            );
        if (getDefaultLanguage() == "el") {
            $option['select'] = 'agent.full_name,agent.image,agent.id,agent.email,agent.date_of_birth,agent.phone_number,agent.create_date,agent.is_active,role.name';
        }
